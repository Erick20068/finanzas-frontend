...
export const NuevoMovimientoModal: React.FC<Props> = ({ onClose, onSuccess }) => {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (cat.length === 0) {
        setError(
          'No hay categorías. Revisa las políticas RLS de la tabla categorias en Supabase (INSERT/SELECT).'
        );
      }
    } catch (e: any) {
      setError(e?.message || 'Error al cargar cuentas/categorías');
    } finally {
  ...
  // Al cambiar tipo, limpiar categoría si no aplica
  useEffect(() => {
    setCategoriaId('');
  }, [tipo]);
  ...
  const categoriasFiltradas = useMemo(() => {
    return categorias.filter((c) => {
      const t = (c.tipo_categoria || '').toUpperCase();
      return !t || t === tipo || t === 'AMBOS' || t === 'AMBOS';
    });
  }, [categorias, tipo]);
    ...
    try {
      ...
      if (tipo === 'TRANSFERENCIA' && cuentaDestinoId) {
        ...
        if (dest) {
          await supabase
            .from('cuentas')
            .update({ saldo_actual: Number(dest.saldo_actual) + valor })
            .eq('id', dest.id);
        }
      }
      onSuccess();
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
      <div className="bg-[#161616] border border-neutral-800 rounded-2xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
              ...
              {(['GASTO', 'INGRESO', 'TRANSFERENCIA'] as const).map((t) => (
                <button
                  ...
                  className={`flex-1 py-2 text-xs font-medium rounded-lg transition ${
                    tipo === t ? 'bg-neutral-700 text-white' : 'text-neutral-400 hover:text-white'
                  }`}
                >
              ...
              >
                ...
                {cuentas.map((c) => (
                  <option key={c.id} value={c.id}>
                    {c.nombre} · ${Number(c.saldo_actual).toFixed(2)}
                  </option>
                ))}
              </select>
              {cuentas.length === 0 && (
                <p className="text-[11px] text-amber-400/90 mt-1">
                  No tienes cuentas. Crea una en la vista Cuentas.
                </p>
              )}
            </div>
            ...
            {tipo !== 'TRANSFERENCIA' && (
              <div>
                ...
                {categoriasFiltradas.length === 0 && (
                  <p className="text-[11px] text-amber-400/90 mt-1">
                    Sin categorías de {tipo.toLowerCase()}.{' '}
